...
export default function UploadPage() {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.id === fileId
              ? { ...f, progress: Math.min(Math.round(progress), 99), status: 'uploading' as const }
              : f
          )
        );
      }
    ...
    (incoming: FileList | File[]) => {
      ...
      for (const file of fileArray) {
        if (!ALLOWED_TYPES.includes(file.type)) {
          setGlobalError(
            `"${file.name}" is not a supported file type. Please upload PDF, DOCX, PNG, JPG, or WebP files.`
          );
          continue;
        }
  ...
  return (
    <motion.div
      className="mx-auto w-full max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8"
    ...
    >
      ...
      <motion.section variants={itemVariants}>
        ...
        <p className="ml-10 text-sm text-sonic-text-secondary sm:text-base text-body">
          Drag & drop your files below or click to browse. Supported formats: PDF, DOCX, PNG, JPG, WebP.
        </p>
      </motion.section>
      ...
      <motion.section variants={itemVariants}>
        ...
        >
          ...
          <p className="mt-3 text-xs text-sonic-text-secondary">
            Max file size: 25 MB &middot; PDF, DOCX, PNG, JPG, WebP
          </p>
        </div>
      </motion.section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                icon: ImageIcon,
                title: 'Image Files',
                desc: 'Support for PNG, JPG, and WebP image uploads.',
                bg: 'bg-violet-50',
                fg: 'text-violet-600',
              },
              {
