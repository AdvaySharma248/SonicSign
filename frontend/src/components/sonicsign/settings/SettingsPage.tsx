...
import type { NotificationSettings, AppearanceSettings, User as SonicSignUser } from '@/types';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.preferences) {
        setAppearance(user.preferences);
      }
    }
  }, [user]);

  const persistProfile = async (updates: Parameters<typeof authApi.updateProfile>[0], successTitle: string) => {
    try {
      setSaving(true);
      const updatedUser = await authApi.updateProfile(updates);
      setUser(updatedUser);
      emitDataEvent(DATA_EVENTS.profileChanged);
      toast({
        title: successTitle,
  ...
  const handleSaveNotifications = () => {
    persistProfile({ notificationSettings: notifications }, 'Notifications updated');
  };

  const handleSaveAppearance = () => {
    persistProfile({ preferences: appearance }, 'Appearance updated');
  };
...
function ProfileSection({
  ...
  onSave: () => void;
  user: SonicSignUser | null;
  saving: boolean;
}) {
  const avatarUrl = user?.photoURL || user?.avatar || '';
  const fallbackName = profileName || user?.email || '';
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              value={(user?.role || 'owner').charAt(0).toUpperCase() + (user?.role || 'owner').slice(1)}
              disabled
